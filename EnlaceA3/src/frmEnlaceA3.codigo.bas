...
Private Sub ConstruirControles()
    ...
    cmbHoja.ControlTipText = "Libros y hojas abiertos en Excel"
    ...
    txtSeriesIncl.ControlTipText = "Series separadas por ; (p.ej. INV;REC). Vacío = todas"
    ...
    ' --- resultado y botones ----------------------------------------------
    Set c = Etiqueta("lblResultado", "RESULTADO", COL_IZQ_ETQ, 318, 120)
    c.Font.Bold = True
    c.ForeColor = COLOR_ACP
    Set txtResultado = Texto("txtResultado", COL_IZQ_ETQ, 334, ancho - 28)
    txtResultado.Height = 110
...
Private Sub EmpresaCambiada()
    ...
    If CargarEmpresa(cod, emp) Then
        ...
        Next i
        If emp.CarpetaSalida <> "" Then txtCarpeta.Text = emp.CarpetaSalida
        mCargando = False
    Else
...
Private Sub LlenarHojas()
    Dim wb As Object, ws As Object, visible As Boolean, seleccion As Long, actual As String
    mNHojas = 0
        ...
        If visible Then
            For Each ws In wb.Worksheets
                If Not (wb.Name = ThisWorkbook.Name And EsHojaConfiguracion(ws.Name)) Then
                    ...
                    cmbHoja.AddItem "[" & wb.Name & "]  " & ws.Name
    ...
    Next wb
    If seleccion > 0 Then
        cmbHoja.ListIndex = seleccion - 1
    ElseIf cmbHoja.ListCount > 0 Then
        cmbHoja.ListIndex = 0
    End If
    mCargando = False
End Sub
